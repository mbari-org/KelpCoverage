    ...
    for i in range(num_slices):
        ...
        if mask_tensor.numel() > 0:
            ...
            if padding_scaled > 0 and content_w > 0 and content_h > 0:
                 ...
                 slice_mask_full = cv2.resize(mask_cpu.astype(np.uint8), (w_new, h_new), interpolation=cv2.INTER_NEAREST).astype(bool)
...
def run_sahi_sam_visualization(
    ...
    if verbose: print(f"Analysis Complete. Saving results...")
    ...
    if generate_slice_viz:
        if isinstance(processor, HierarchicalProcessor):
            if verbose: print("Generating fine-pass slice visualization...")
