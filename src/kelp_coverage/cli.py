...
def _run_analysis(args: argparse.Namespace) -> None:
    ...
    if not args.verbose and not args.site:
        with tqdm(total=len(sites_to_process), desc="Overall Progress") as pbar:
            for site in sites_to_process:
                ...
                pbar.set_description(f"Processing Site: {site}")
...
def main() -> None:
    ...
    # --- Base Model & Processing Arguments ---
    ...
    base_parser.add_argument('--threshold', type=int, default=20, help='Threshold for LAB color distance to identify water pixels.')
    base_parser.add_argument('--threshold-max', type=int, default=20, help='Maximum LAB color threshold to search up to if no points are found.')
    ...
    base_parser.add_argument('--uniform-grid-thresh', type=float, default=0.85, help='Percentage of uniform grids required to shortcut SAM.')
    base_parser.add_argument('--water-grid-thresh', type=float, default=0.95, help='Percentage of water-colored grids required to shortcut SAM.')

    base_parser.add_argument('--fallback-brightness-threshold', type=float, default=100.0, help='[FALLBACK] Brightness threshold to classify a slice as water if no points are found.')
    base_parser.add_argument('--fallback-distance-threshold', type=float, default=55.0, help='[FALLBACK] LAB color distance threshold to classify a slice as water if no points are found.')
    ...
    analyze_parser.add_argument('--use-erosion-merge', action=argparse.BooleanOptionalAction, default=True, help='[HIERARCHICAL] Use erosion on the coarse mask by default. Use --no-use-erosion-merge to disable.')
    analyze_parser.add_argument('--erosion-kernel-size', type=int, default=51, help='[HIERARCHICAL] Kernel size for the erosion merge.')
    ...
    analyze_parser.add_argument('--use-color-validation', action=argparse.BooleanOptionalAction, default=True, help='[HIERARCHICAL] Use color validation by default. Use --no-use-color-validation to disable.')
    ...
    analyze_parser.add_argument('--generate-merge-viz', action='store_true', help='[HIERARCHICAL] Generate a heatmap visualization of the disagreement area during mask merging.')
    analyze_parser.add_argument('--merge-color-threshold', type=int, default=15, help='[HIERARCHICAL] LAB color distance threshold for validating kelp in the merge disagreement zone.')
    analyze_parser.add_argument('--merge-lightness-threshold', type=float, default=75.0, help='[HIERARCHICAL] Lightness (L*) threshold for validating kelp in the merge disagreement zone.')
    ...
    # Display Options
    heatmap_parser.add_argument('--verbose', '-v', action='store_true', help='Verbose output')      
