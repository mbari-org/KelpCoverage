...
def generate_heatmap(coverage_csv: str, site_prefix: str,
    ...
    grouped = intersection.groupby('grid_id').agg(
        weighted_cov_sum=('weighted_cov', 'sum'),
        area_sum=('area', 'sum')
    )
    ...
    if grid_to_plot.empty:
        print(f"No data to plot for site prefix '{site_prefix}'. Skipping heatmap generation.")
        return
    ...
    if show_grid_values:
        for _, row in grid_to_plot.iterrows():
            ...
            ax.text(centroid.x, centroid.y, value_text,
                    ha='center', va='center', color='white', fontsize=8,
                    bbox=dict(facecolor='black', alpha=0.4, edgecolor='none'))

    if show_point_labels:
        for _, row in gdf_coverage_proj.iterrows():
            plt.annotate(text=row['num_id'], xy=(row.geometry.x, row.geometry.y),
